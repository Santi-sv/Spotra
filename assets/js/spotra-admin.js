...
(function(){
  ...
  async function loadApprovals(){
    ...
    try {
      ...
      if(subTabs) subTabs.insertAdjacentHTML('afterend', subs.map(rowHTML).join(''));
      setCount(v, subs.length);
    } finally {
      loading = false;
    }
  }
  ...
  async function handleReview(row, decision){
    ...
    const name = (row.querySelector('.name') || {}).textContent || 'Lugar';
    row.querySelectorAll('button').forEach(b => b.disabled = true);

    const result = window.SpotraBackend
      ? await window.SpotraBackend.reviewSubmission(id, decision)
      : { ok: false };
    ...
    notify(decision === 'approved'
      ? `"${name.trim()}" aprobado. Ya aparece en el mapa.`
      : `"${name.trim()}" rechazado.`);
  }

  /* Intercepta Aprobar/Rechazar de filas reales ANTES del handler legacy */
  document.addEventListener('click', function(e){
    const row = e.target.closest('.approve-row[data-sub-id]');
    if(!row) return;
