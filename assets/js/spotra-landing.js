...
(function(){
  ...
  /* ===== landing de marcas — calle cruda ===== */
  + '.sg-brand{position:relative;z-index:1;--hueso:#EDE9E1;--cinta:#FFD400;--hormigon:#16181A;color:var(--hueso);}'
  + '.sg-brand .sg-wrapinner{max-width:1180px;margin:0 auto;}'
  + '.sg-noise{position:fixed;inset:0;pointer-events:none;z-index:0;opacity:.16;mix-blend-mode:overlay;background-image:url("data:image/svg+xml;utf8,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' width=\'160\' height=\'160\'%3E%3Cfilter id=\'n\'%3E%3CfeTurbulence type=\'fractalNoise\' baseFrequency=\'0.85\' numOctaves=\'3\'/%3E%3CfeColorMatrix type=\'saturate\' values=\'0\'/%3E%3C/filter%3E%3Crect width=\'160\' height=\'160\' filter=\'url(%23n)\' opacity=\'0.5\'/%3E%3C/svg%3E");background-size:160px 160px;}'
  /* barra */
  + '.sg-topnav{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:18px 0 14px;margin-bottom:8px;border-bottom:2px solid rgba(237,233,225,.14);}'
  + '.sg-topnav .brandmark{font-family:\"Clash Display\",\"General Sans\",sans-serif;font-weight:700;font-size:20px;letter-spacing:.14em;color:var(--hueso);}'
  + '.sg-topnav .brandmark span{color:#2ee84d;}'
  + '.sg-topnav nav{display:none;gap:26px;}'
  + '.sg-topnav nav a{color:#8d918a;font-size:12px;letter-spacing:.16em;text-transform:uppercase;font-weight:600;text-decoration:none;}'
  + '.sg-topnav nav a.on{color:var(--hueso);box-shadow:inset 0 -8px 0 rgba(46,232,77,.35);}'
  + '.sg-navcta{background:var(--cinta);color:#111;border:0;font-family:inherit;font-size:12px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;padding:11px 16px;cursor:pointer;white-space:nowrap;transform:rotate(-1.2deg);box-shadow:4px 4px 0 rgba(0,0,0,.55);}'
  /* hero */
  + '.sg-hero{display:grid;gap:30px;padding:26px 0 10px;}'
  + '.sg-eyebrow{display:inline-block;background:#2ee84d;color:#06210c;font-size:11px;font-weight:800;letter-spacing:.2em;text-transform:uppercase;padding:6px 12px;transform:rotate(-1.5deg);}'
  + '.sg-hero-h1{font-family:\"Clash Display\",\"General Sans\",sans-serif;font-weight:700;text-transform:uppercase;font-size:clamp(40px,11vw,74px);line-height:.92;letter-spacing:-.025em;margin:16px 0 18px;color:var(--hueso);text-shadow:0 4px 24px rgba(0,0,0,.85);}'
  + '.sg-hero-h1 em{font-style:normal;display:block;color:#2ee84d;}'
  + '.sg-hero-h1 .mark{position:relative;display:inline-block;}'
  + '.sg-hero-h1 .mark:after{content:"";position:absolute;left:-2%;right:-2%;bottom:.1em;height:.16em;background:var(--cinta);opacity:.85;transform:rotate(-1deg);z-index:-1;}'
  + '.sg-hero-lead{color:#b4b8b0;font-size:16px;line-height:1.5;margin:0 0 24px;max-width:540px;border-left:3px solid rgba(46,232,77,.5);padding-left:14px;}'
  + '.sg-ctas{display:flex;flex-wrap:wrap;gap:12px;}'
  + '.sg-cta-main{background:#2ee84d;color:#06210c;border:0;font-family:inherit;font-weight:800;font-size:15px;letter-spacing:.06em;text-transform:uppercase;padding:16px 24px;cursor:pointer;box-shadow:5px 5px 0 #0a0b0a,5px 5px 0 1px rgba(237,233,225,.25);transform:rotate(-.8deg);}'
  + '.sg-cta-main:active{transform:translate(2px,2px) rotate(-.8deg);box-shadow:2px 2px 0 #0a0b0a;}'
  + '.sg-cta-ghost{background:none;color:var(--hueso);border:2px solid rgba(237,233,225,.35);font-family:inherit;font-weight:700;font-size:15px;letter-spacing:.06em;text-transform:uppercase;padding:15px 22px;cursor:pointer;}'
  + '.sg-tape{display:inline-block;margin-top:30px;background:var(--cinta);color:#111;font-family:\"Clash Display\",\"General Sans\",sans-serif;font-weight:700;font-size:13px;letter-spacing:.08em;text-transform:uppercase;padding:8px 18px;transform:rotate(-2deg);box-shadow:3px 3px 0 rgba(0,0,0,.5);}'
  + '.sg-quote{margin-top:22px;color:#cfd2c9;font-size:15px;line-height:1.45;font-family:\"Clash Display\",\"General Sans\",sans-serif;font-weight:500;text-transform:uppercase;letter-spacing:.02em;}'
  + '.sg-quote small{display:block;margin-top:10px;color:#6c716a;font-size:10px;letter-spacing:.24em;}'
  /* tarjetas de beneficio */
  + '.sg-cards{display:grid;gap:14px;margin:0;padding:0;list-style:none;}'
  + '.sg-cards li{position:relative;overflow:hidden;background:var(--hormigon);border:2px solid rgba(237,233,225,.1);padding:20px 18px 18px 20px;}'
  + '.sg-cards li:nth-child(odd){transform:rotate(-.6deg);}'
  + '.sg-cards li:nth-child(even){transform:rotate(.5deg);}'
  + '.sg-cards li:before{content:attr(data-n);position:absolute;right:-6px;top:-22px;font-family:\"Clash Display\",\"General Sans\",sans-serif;font-size:92px;font-weight:700;line-height:1;color:transparent;-webkit-text-stroke:2px rgba(237,233,225,.09);pointer-events:none;}'
  + '.sg-cards .ico{width:40px;height:40px;display:flex;align-items:center;justify-content:center;background:#2ee84d;margin-bottom:12px;transform:rotate(-2deg);}'
  + '.sg-cards .ico svg{width:21px;height:21px;color:#06210c;}'
  + '.sg-cards b{display:block;font-family:\"Clash Display\",\"General Sans\",sans-serif;font-size:19px;text-transform:uppercase;letter-spacing:-.01em;margin-bottom:6px;color:var(--hueso);}'
  + '.sg-cards p{margin:0;color:#9ba09a;font-size:14px;line-height:1.5;max-width:44ch;}'
  /* oferta */
  + '.sg-offerbar{position:relative;display:grid;gap:12px;margin:40px 0 0;background:#2ee84d;color:#06210c;padding:26px 22px;transform:rotate(-.7deg);box-shadow:8px 8px 0 rgba(0,0,0,.55);}'
  + '.sg-offerbar .tag{font-size:11px;letter-spacing:.22em;text-transform:uppercase;font-weight:800;opacity:.8;}'
  + '.sg-offerbar .price{font-family:\"Clash Display\",\"General Sans\",sans-serif;font-size:clamp(42px,12vw,68px);font-weight:700;line-height:.9;letter-spacing:-.03em;margin:6px 0 0;color:#07130a;}'
  + '.sg-offerbar p{margin:0;font-size:14.5px;line-height:1.5;font-weight:500;max-width:46ch;}'
  /* seccion final */
  + '.sg-final{display:grid;gap:26px;margin-top:44px;}'
  + '.sg-bullets{display:grid;gap:16px;margin:22px 0 0;padding:0;list-style:none;}'
  + '.sg-bullets li{display:grid;grid-template-columns:38px 1fr;gap:14px;align-items:start;}'
  + '.sg-bullets .ico{width:38px;height:38px;display:flex;align-items:center;justify-content:center;border:2px solid #2ee84d;transform:rotate(-2deg);}'
  + '.sg-bullets .ico svg{width:19px;height:19px;color:#2ee84d;}'
  + '.sg-bullets b{display:block;font-family:\"Clash Display\",\"General Sans\",sans-serif;font-size:16px;text-transform:uppercase;margin-bottom:3px;color:var(--hueso);}'
  + '.sg-bullets p{margin:0;color:#9ba09a;font-size:14px;line-height:1.5;}'
  + '.sg-brand .sg-card{background:var(--hormigon);border:2px solid rgba(237,233,225,.12);border-radius:0;box-shadow:8px 8px 0 rgba(0,0,0,.5);}'
  + '.sg-brand .sg-card h2{font-family:\"Clash Display\",\"General Sans\",sans-serif;text-transform:uppercase;font-size:22px;letter-spacing:-.01em;}'
  + '.sg-brand .sg-field input,.sg-brand .sg-field select{border-radius:0;border:2px solid rgba(237,233,225,.16);background:#0e1010;}'
  + '.sg-brand .sg-btn{border-radius:0;text-transform:uppercase;letter-spacing:.06em;font-weight:800;box-shadow:5px 5px 0 rgba(0,0,0,.5);}'
  + '.sg-brand .sg-ig{border-radius:0;border:2px solid rgba(237,233,225,.18);}'
  + '.sg-brand .sg-demo-head h3{text-transform:uppercase;font-size:clamp(30px,8vw,46px);line-height:.95;letter-spacing:-.02em;}'
  ...
  +   '.sg-cards li{padding:24px 22px;}'
  +   '.sg-cards li:before{font-size:120px;top:-30px;}'
  +   '.sg-offerbar{grid-template-columns:auto 1fr;gap:34px;align-items:center;padding:30px 34px;}'
  ...
  function brandBody(){
    ...
    var cards = b('benefits').map(function(x, i){
      return '<li data-n="0' + (i + 1) + '"><div class="ico">' + icon(icons[i] || 'pin') + '</div>'
        + '<b>' + esc(x[0]) + '</b><p>' + esc(x[1]) + '</p></li>';
    }).join('');
    ...
    return '<div class="sg-brand"><div class="sg-noise"></div><div class="sg-wrapinner">'
    + '<div class="sg-topnav">'
    +   '<div class="brandmark">SPOT<span>RA</span></div>'
    ...
    +   '<div>'
    +     '<span class="sg-eyebrow">' + esc(b('kicker')) + '</span>'
    ...
    +     '</div>'
    +     '<div class="sg-tape">' + esc(b('quoteFoot')) + '</div>'
    +     '<div class="sg-quote">' + esc(b('quote')) + '</div>'
    +   '</div>'
    ...
    +   '<div>'
    +     '<span class="sg-eyebrow">' + esc(b('joinKicker')) + '</span>'
    +     '<h2 class="sg-hero-h1" style="font-size:clamp(32px,8vw,52px)">' + esc(b('formTitle')) + '</h2>'
